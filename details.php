<?php
    ...
    if (isset($_POST['confirm'])) {
            //CONFIRMING THE CHOSEN PRODUCTS THUS TAKING THEM TO CART 

		$product = $arrProducts[$_GET['name']]['product1'];
        $product2 = $arrProducts[$_GET['name']]['product2'];
        $description = $arrProducts[$_GET['name']]['description'];
 		$name = $arrProducts[$_GET['name']]['name'];
        $product_price = $arrProducts[$_GET['name']]['product_price'];
		
 		$Qty = $_POST['quantity'];
        $size = $_POST['radSize'];

 		$addQty = $_SESSION['totalQuantity'] + $Qty;		
		$_SESSION['totalQuantity'] = $addQty;
		...
		if(isset($_SESSION['arrCart'])) {
			...
			if ($CartQty == 1) {
				...
				$addArrcart = ['name'=>''.$name, 'quantity'=>''.$Qty, 'size'=>''.$size, 'product'=>''.$product, 'product_price'=>''.$product_price, 'product2'=>''.$product2, 'description'=>''.$description];
	...
	<div class="container">
		<form method="post">	
		    <h3 class="h3 text-left"><i class="fas fa-store"></i> Learn IT Easy Online Shop
		    	...
		    	GIT	<i class="fas fa-shopping-cart"></i> Cart <span class="badge badge-light">
		        ...
		        <div class="col-md-4 col-sm-6">
		            <div class="product-grid2">
		                <div class="product-image2">
		                        <img class="pic-1" height="70%" src="IMG/<?php echo $arrProducts[$_GET['name']]['product1'];?>">
		                        <img class="pic-2" height="70%" src="IMG/<?php echo $arrProducts[$_GET['name']]['product2'];?>">
		                </div>
		            </div>
		        </div>
		        <div class="col-md-8 col-sm-6">
                    <!-- display product name and price -->
		        	<h3><?php echo $arrProducts[$_GET['name']]['name'];?> <span class="pricebg"><?php echo " ₱ ".$arrProducts[$_GET['name']]['product_price'];?></span></h3>
